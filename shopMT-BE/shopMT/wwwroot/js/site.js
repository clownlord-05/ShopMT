...
function getBotResponse(input) {
    ...
    if (input.includes("gaming") || input.includes("đồ họa") || input.includes("chơi game") || input.includes("lập trình")) {
        return "Các dòng laptop hiệu năng cao như ASUS ROG STRIX, ASUS TUF Gaming, Lenovo Legion rất phù hợp. Giá từ 17–35 triệu, có card đồ họa rời.";
    }
    ...
    if (input.includes("rog") || input.includes("strix g15") || input.includes("asus rog")) {
        return "ASUS ROG Strix G15 - Ryzen 7 5800H, RAM 16GB, SSD 512GB, RTX 3060, màn 15.6\" 144Hz. Giá KM: 25.990.000đ (giá gốc 29.990.000đ).";
    }

    if (input.includes("màn hình lg") || input.includes("ultragear 27") || input.includes("lg gaming")) {
        return "Màn hình LG UltraGear 27\" QHD, tần số quét 165Hz, phản hồi 1ms. Giá: 8.990.000đ (giá gốc 10.990.000đ).";
    }

    if (input.includes("i7-12700k") || input.includes("intel 12700k") || input.includes("cpu 12700k")) {
        return "CPU Intel Core i7-12700K - 12 nhân, 20 luồng, boost 5.0GHz. Giá KM: 9.990.000đ.";
    }

    if (input.includes("xps 13") || input.includes("dell xps")) {
        return "Dell XPS 13 - Core i7-1165G7, RAM 16GB, SSD 512GB, màn FHD+, chỉ 1.2kg. Giá: 32.990.000đ.";
    }

    if (input.includes("spectre") || input.includes("hp spectre")) {
        return "HP Spectre x360 14 - Core i7-1355U, RAM 16GB, SSD 1TB, màn OLED cảm ứng 3K2K. Giá: 33.990.000đ.";
    }

    if (input.includes("macbook air m2") || input.includes("mac air m2")) {
        return "MacBook Air M2 13.6\" (2023) - RAM 8GB, SSD 256GB, siêu mỏng nhẹ. Giá: 28.990.000đ.";
    }

    if (input.includes("yoga slim 7i") || input.includes("lenovo yoga slim")) {
        return "Lenovo Yoga Slim 7i Carbon Gen 7 - Core i7-1260P, RAM 16GB, SSD 1TB, chỉ 0.97kg. Giá: 30.990.000đ.";
    }

    if (input.includes("rtx 4060") || input.includes("msi 4060")) {
        return "Card đồ họa MSI GeForce RTX 4060 Ventus 2X - VRAM 8GB, bus 128-bit. Giá: 8.390.000đ.";
    }

    if (input.includes("corsair vengeance") || input.includes("ram corsair")) {
        return "RAM Corsair Vengeance LPX 16GB DDR4 3200MHz - giá chỉ 1.290.000đ.";
    }

    if (input.includes("ultragear 27gn750") || input.includes("màn hình lg 240hz")) {
        return "LG UltraGear 27GN750-B 27\" Full HD 240Hz - giá 5.990.000đ.";
    }

    if (input.includes("asus tuf vg249q1a") || input.includes("tuf gaming 23.8")) {
        return "ASUS TUF Gaming VG249Q1A 23.8” IPS 165Hz - giá 4.290.000đ.";
    }

    if (input.includes("gskill trident z") || input.includes("ram rgb gskill")) {
        return "RAM G.SKILL Trident Z RGB 16GB DDR4 3200MHz - giá 1.790.000đ.";
    }

    // Gợi ý theo loại sản phẩm
    if (input.includes("màn hình") || input.includes("monitor")) {
        return "Hiện có LG UltraGear 27\", LG 240Hz, ASUS TUF 165Hz... Bạn muốn màn Full HD hay QHD?";  
    }

    if (input.includes("card đồ họa") || input.includes("gpu") || input.includes("vga")) {
        return "Có MSI RTX 4060, RTX 3060... Bạn chơi game hay làm đồ họa để mình tư vấn loại phù hợp.";
    }

    if (input.includes("ram") && !input.includes("gskill") && !input.includes("corsair")) {
        return "Có RAM Corsair 16GB DDR4 3200MHz và G.SKILL Trident Z RGB 16GB DDR4 3200MHz. Bạn thích loại có RGB hay không?";
    }
    if (input.includes("hỗ trợ trực tiếp")) {
        return "Bạn có thể liên hệ qua message và zalo để chúng tôi có thể trợ giúp bạn kịp thời và nhanh chóng nhất";
    }
    if (inout.includes("zalo") || inout.includes("liên hệ zalo")) {
        return "Bạn hãy liên hệ zalo bên dưới";
    }
